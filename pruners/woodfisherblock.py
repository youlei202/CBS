...
class BlockwiseWoodburryFisherPruner(GradualPruner):
    ...
    def _compute_split_block_woodburry_fisher_inverse(self, param_idx, start, end, device):
        ...
        for idx in range(self._num_grads):
            ...
            if idx == 0:
                ...
                _sub_block_fisher_inv = torch.ger(sample_grads, sample_grads)
                _sub_block_fisher_inv.mul_(1.0 / numerator_normalization).div_(
        ...
        if not self.args.fisher_split_grads or param.numel() <= self._fittable_params:
            for idx in range(self._num_grads):
                ...
                if idx == 0:
                    ...
                    _block_fisher_inv = torch.ger(sample_grads, sample_grads)\
                            .mul_(1.0 / numerator_normalization)\
                            .div_(self._goal + (sample_grads.dot(sample_grads) / self.args.fisher_damp))
                    ##_block_fisher_inv = torch.ger(sample_grads, sample_grads)
                    #_block_fisher_inv = sample_grads[:,None] @ sample_grads[None,:]
                    #_block_fisher_inv.mul_(1.0 / numerator_normalization)
                    #ss = (self._goal + (sample_grads.dot(sample_grads) / self.args.fisher_damp))
                    #_block_fisher_inv.div_(ss)
                    #print('----------------------')
                    #import pdb;pdb.set_trace()
                    #k = _block_fisher_inv.size(0)
                    #_block_fisher_inv.as_strided([k], [k + 1]).sub_(1.0 / self.args.fisher_damp)
                    ...
                    #cache_matmul = product_efficient_v1(_block_fisher_inv, sample_grads, num_parts=2)
                    cache_matmul.div_((self._goal + sample_grads.dot(cache_matmul)) ** 0.5)
    ...
    def _dump_fisher_inv(self, idx, _param_idx):
        #import pdb;pdb.set_trace()
        layer_fisher_inv_dict_path = os.path.join( 
                self.args.fisher_inv_path, f'block_fisher_inv_{idx}.pkl')
        if not os.path.exists(layer_fisher_inv_dict_path):
            layer_fisher_inv_dict = {'layer_start': _param_idx, 
                    'f_inv': self._block_fisher_inv_dic[_param_idx]}
            #import pdb;pdb.set_trace()
    ...
    def _load_fisher_inv(self, device, idx, _param_idx):
        layer_fisher_inv_dict_path = os.path.join( 
                self.args.fisher_inv_path, f'block_fisher_inv_{idx}.pkl')
        print(layer_fisher_inv_dict_path)
        if os.path.exists(layer_fisher_inv_dict_path):
            #import pdb;pdb.set_trace()
            try:
                data = _load(layer_fisher_inv_dict_path)
            except:
                return
            self._block_fisher_inv_dic[_param_idx] = data['f_inv']
            diag = []
            if type(data['f_inv']) == list:
                for m in data['f_inv']:
                    diag.append(m.diagonal())
                diag = torch.cat(diag)
            else:
                diag = data['f_inv'].diagonal()
            self._block_fisher_inv_diag_dic[_param_idx] = diag
            self._total_block_fisher_params += len(self._block_fisher_inv_diag_dic[_param_idx])

    def _dump_all_grads(self):
        layer_fisher_inv_dict_path = os.path.join(
                self.args.fisher_inv_path, f'all_grads.pkl')
        if not os.path.exists(layer_fisher_inv_dict_path):
            _dump(layer_fisher_inv_dict_path, {'all_grads': self._all_grads})
    ...
    def _get_weight_update(self, param_idx, scaled_basis_vector, idx):
        scaled_basis_device = scaled_basis_vector.device
        #self._load_fisher_inv(scaled_basis_device, idx, param_idx)
        ...
        #import pdb;pdb.set_trace()
        #self._block_fisher_inv_dic.pop(param_idx, None) 
        #torch.cuda.empty_cache()
        #import pdb;pdb.set_trace()
    ...
    def prune_to_level(self, desired_level, dset, subset_inds, device, num_workers, epoch_num, **kwargs):
        ...
        for idx, module in enumerate(self._modules):
            ...
            #layer_fisher_inv_dict_path = os.path.join( self.args.run_dir, f'sparsity{desired_level}_block_fisher_inv_{idx}.pkl')
            #with open(layer_fisher_inv_dict_path, 'wb') as f_:
            #    pickle.dump(layer_fisher_inv_dict, f_, pickle.HIGHEST_PROTOCOL)
            #import pdb;pdb.set_trace()
            ...
            self._block_fisher_inv_dic.pop(self._param_idx, None)
            torch.cuda.empty_cache()
        ...
        if self.args.woodburry_joint_sparsify:
            ...
            mask_dict = {'mask': global_param_mask, 'sparsity': desired_level}
            mask_dict_path = os.path.join(
                    #self.args.run_dir, f'global_mask_sparsity{desired_level}.pkl')
                    #self.args.greedy_dir, f'global_mask_sparsity{desired_level}.pkl')
                    self.args.fisher_inv_path, f'global_mask_sparsity{desired_level}.pkl')
            ...
            for idx, module in enumerate(self._modules):
                ...
                weight_update = self._get_weight_update(_param_count, scaled_basis_vector, idx)
                ...
                #Just for ablation study of selection and weight adjustment
    ...
    def _debug_update_w_with_comb(self, desired_level):
        #from greedy_alg.greedyblock import GreedyBlockPruner
        self.greedy_pruner = GreedyBlockPruner(sparsity=desired_level, wgh_path=self.args.wgh_path, device='cuda', args=self.args)
        delta_w = greedy_pruner.cal_delta_w_not_comb()
        #import pdb;pdb.set_trace()
        return delta_w
