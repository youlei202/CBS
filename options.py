...
def get_parser():
    ...
    parser.add_argument('--not-update-weights', action='store_true', dest='not_update_weights', 
            help='not update weights after pruning ')
    parser.add_argument('--update-weights', action='store_false', dest='not_update_weights', 
            help='update weights after pruning ')
    parser.add_argument('--regressor-result-path', type=str, default=None, help='the path of the regressor results for combinatorial pruning')
    parser.add_argument('--comb-method', type=str, default='ep', 
                choices = ['ep', 'elastic_net'],
                help='the path of the regressor results for combinatorial pruning')
    parser.add_argument('--greedy-iter', type=int, default=0, help='how many iterations the greedy algorithm run')
    ...
    parser.add_argument('--idx-2-module-path', type=str, default=None, help='the path where to load idx_2_module file')
    parser.add_argument('--wgh_path', type=str, default=None, help='the path where to load/save wgh file')
    parser.add_argument('--fisher_inv_path', type=str, default=None, help='the path where to load fisher_inv')
    parser.add_argument('--greedy_method', default='greedyblock', choices = ['greedy', 'greedyblock', 'greedylayer'],
                        help='greedy or greedyblock pruning')
    parser.add_argument('--init_method', default='mag', choices = ['mag', 'wg', 'woodfisherblock', 'random', 'mag_perb'],
    ...
    parser.add_argument('--max-mag-perb', default=50, type=int,
    ...
    parser.add_argument('--swap-one-per-iter', dest='swap_one_per_iter', action='store_true',
                        help='this is not related to the algorithm and is used to run experiments')
    parser.add_argument('--not-swap-one-per-iter', dest='swap_one_per_iter', action='store_false',
                        help='this is not related to the algorithm and is used to run experiments')
    parser.set_defaults(swap_one_per_iter=False)
    parser.add_argument('--max-N-fluctation', default=7, type=int,
                        help='Stop online greedy when sample loss of last N iterations become stable')
   

    # Flags for evaluate the greedy results
    parser.add_argument('--greedy-path', type=str, default=None, 
    ...
    parser.add_argument('--when-to-greedy', type=str, 
                        choices=['online', 'offline'], default='online', 
                        help='After training, run greedy algorithm (online) or load its results from files (offline)')
    # for online greedy
    parser.add_argument('--keep-pruned', dest='keep_pruned', action='store_true',
                        help = 'keep the weights before pruning stored')
    parser.add_argument('--not-keep-pruned', dest='keep_pruned', action='store_false',
                        help = 'keep the weights before pruning not stored')
    parser.set_defaults(keep_pruned=False)

    parser.add_argument('--weight_update_method', type=str, 
                        choices=['single', 'multiple'], default='multiple', 
                        help='calculate the weight updating consider removing one weight (single) or multiple weights (multiple)')
    parser.add_argument('--update-weight-online', action='store_true',
                        help = 'After training, calculate the weight update (default: False)')
    parser.add_argument('--ablation-study-update-weight', action='store_true',
                        help = 'Debug: update remaining weights with removing single/mulitple weights for the baselines (mag, woodfisher)')
    
    parser.add_argument('--has_first_order_term_in_obj', action='store_true',
